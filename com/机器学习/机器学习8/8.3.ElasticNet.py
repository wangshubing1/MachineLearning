...
if __name__ == "__main__":
    ...
    N = 9
    x = np.linspace(0, 6, N) + np.random.randn(N)#np.random.randn(N)：高斯噪声
    print(x)
    print("**********************")
    x = np.sort(x)
    ...
    print (np.logspace(-3, 2, 5))
    print(np.logspace(-2, 9, 5))
    print(np.logspace(0, 0, 5))
    ...
    print("==================",m)
    clrs = []  # 颜色
    print(np.linspace(16711680, 255, m))
    for c in np.linspace(16711680, 255, m):
        clrs.append("#%06x" % int(c))

    line_width = np.linspace(5, 2, m)
    titles = '线性回归', u'Ridge回归', u'LASSO', u'ElasticNet'
    tss_list = []
    rss_list = []
    ess_list = []
    ess_rss_list = []
